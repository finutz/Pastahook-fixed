...
void renderESPpreview(ImDrawList* draw, ImVec2 pos) noexcept
{
    ...
    auto sizeMed = ImGui::CalcTextSize(skCrypt("Pastahook"));
    ImGui::PopFont();
    ImGui::PushFont(gui->weaponIcons());
    ...
    if (currentCategory == 0)
    {
        ...
        {
            draw->AddText(gui->espFont(), 12.f, ImVec2{pos.x + 185 / 2 - (sizeMed.x / 2), pos.y + 25}, Helpers::calculateColor(config->esp.enemy.name), c_xor("Pastahook"));
        }
    ...
    if (currentCategory == 1)
    {
        ...
        {
            draw->AddText(gui->espFont(), 12.f, ImVec2{ pos.x + 185 / 2 - (sizeMed.x / 2), pos.y + 25 }, Helpers::calculateColor(config->esp.allies.name), c_xor("Pastahook"));
        }
...
void GUI::renderVisualsWindow() noexcept
{
   ...
   // }
    ...
    {
        ImGui::SameLine();
        ImGui::PushID("?1'thirdperson'1?tf is u looking in hex for 'Pastahook");
        if (ImGui::Button("..."))
            ImGui::OpenPopup("ttt");
...
void GUI::renderMovementWindow() noexcept
{
    ...
    ImGui::SameLine();
    ImGui::PushID("stop using hex u dirty hoe, Pastahook");
    if (ImGui::Button("..."))
        ImGui::OpenPopup("");
    if (ImGui::BeginPopup(""))
    {
        ...
        ImGui::Combo(c_xor("Sound"), &config->misc.ebdetect.sound, skCrypt("None\0Pastahook\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Custom\0"));      
...
void GUI::renderMiscWindow() noexcept
{
    ...
    ImGui::Combo(c_xor("Hit Sound"), &config->misc.hitSound, skCrypt("None\0Pastahook\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Shield\0Clock\0Custom\0"));
    ...
    ImGui::PushID(2);
    ImGui::Combo(c_xor("Kill Sound"), &config->misc.killSound, skCrypt("None\0Pastahook\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Shield\0Clock\0Custom\0"));
...
void GUI::renderGuiStyle() noexcept
{
    ...
    static auto Name = skCrypt("Pastahook");
            ...
            ImGui::BeginChild(skCrypt("##Main"), ImVec2{ 750 - 70, 555 }, false);
            {
                ...
                if (text == " Pastahook")
                {
                    ...
                    ImVec2 size = ImGui::GetWindowSize();
                    {
                        ...
                        draw->AddText(fonts.tahoma34, 24.f, ImVec2(pos.x + 26.5f + 1, pos.y + 2), Helpers::calculateColor(config->menu.accentColor, 0.5f), skCrypt(" Pastahook"));
                        draw->AddText(fonts.tahoma34, 24.f, ImVec2(pos.x + 26.5f, pos.y + 1), ImColor(255.f, 255.f, 255.f, 255.f), skCrypt(" Pastahook"));
                        ImGui::SameLine();
