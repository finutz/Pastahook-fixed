        ...
        if (show && config->misc.jumpStats.enabled)
        {
            ...
            if (jumps > 2)
                memory->clientMode->getHudChat()->printf(0,
                    skCrypt(" [Pastahook] %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%d\x01 Bhops | \x05%.0f\x01 Sync]"),
                    color, jump.c_str(),
                ...
                memory->clientMode->getHudChat()->printf(0,
                    skCrypt(" [Pastahook] %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%.0f\x01 Sync]"),
                    color, jump.c_str(),
...
void Misc::ebdetections(UserCmd* cmd)
{
    ...
    if (VelocitiesForDetection.size() == 2 && ((round(negativezvel * 100.f) == round(VelocitiesForDetection.at(0).velocity.z * 100.f) && VelocitiesForDetection.at(1).velocity.z < negativezvel && !VelocitiesForDetection.at(1).onground && !VelocitiesForDetection.at(0).onground) || detectdata.edgebugtick == memory->globalVars->tickCount))
    {
        ...
        if (config->misc.ebdetect.chat)
            memory->clientMode->getHudChat()->printf(0, skCrypt("[Pastahook] Edge bugged"));
...
void Misc::Indictators() noexcept
{
    ...
    if (config->misc.indicators.style == 1)
    {
        int offset = 1;
        {
            ...
            std::string text1 = c_xor("Pasta");
            interfaces->surface->printText(std::wstring(text1.begin(), text1.end()));
            auto textSize = interfaces->surface->getTextSize(hooks->smallFonts, L"Pasta");
            ...
            std::string text2 = c_xor("hook");
            interfaces->surface->printText(std::wstring(text2.begin(), text2.end()));
            offset += 1;
        }
        interfaces->surface->setDrawColor(255, 255, 255, 255);
        interfaces->surface->drawLine(s.x / 2 + 7, s.y / 2 + interfaces->surface->getTextSize(hooks->smallFonts, L"Pastahook").second + 9, s.x / 2 + interfaces->surface->getTextSize(hooks->smallFonts, L"Pastahook").first + 7, s.y / 2 + 9 + interfaces->surface->getTextSize(hooks->smallFonts, L"Pastahook").second);
        if ((config->misc.indicators.toShow & 1 << ITriggerBot) == 1 << ITriggerBot)
        {
            if (config->triggerbotKey.isActive() && config->lgb.enableTriggerbot)
            {
...
void Misc::updateClanTag(bool tagChanged) noexcept
{
    ...
    if (config->misc.clantag)
    {
        ...
        if (time != lastTime)
        {
            switch (time % 2)
            {
            case 0: { memory->setClanTag(skCrypt(" pasta "), skCrypt(" hook ")); break; }
            case 1: { memory->setClanTag(skCrypt(" pasta "), skCrypt(" hook ")); break; }
            }
...
void Misc::watermark() noexcept
{
    ...
    std::ostringstream text; text << c_xor("Pastahook") << (config->misc.wm.showUsername ? " | " + namey : "") << (config->misc.wm.showTime ? " | " + fuck : "") << (config->misc.wm.showFps ? " | " + fps : "") << (config->misc.wm.showPing ? " | " + ping : "") << (interfaces->engine->isConnected() && config->misc.wm.showTicks ? " | " + std::to_string(tick) + " Ticks" : "");
...
void Misc::voteRevealer(GameEvent& event) noexcept
{
    ...
    if (!memory->clientMode->getHudChat())
        return;
    memory->clientMode->getHudChat()->printf(0, skCrypt(" Pastahook | %s voted %c%s"), isLocal ? localPlayer.get()->getPlayerName().c_str() : entity->getPlayerName().c_str(), color, !votedYes ? "Yes" : "No");
}
...
void Misc::onVoteStart(const void* data, int size) noexcept
{
    ...
    std::string cock = c_xor("Pastahook | ") + entity->getPlayerName() + c_xor(" called a vote to ") + voteName(voteType);
...
void Misc::onVotePass() noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, skCrypt("Pastahook | Vote passed"));
...
void Misc::onVoteFailed() noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, skCrypt("Pastahook | Vote failed"));
